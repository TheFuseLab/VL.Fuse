...
public class SwitchBoolean<T> : ResultNode<T>
{
    ...
    public SwitchBoolean(NodeContext nodeContext, ShaderNode<bool> inCheck, ShaderNode<T> inFalse, ShaderNode<T> inTrue,
        ShaderNode<T> theDefault = null) : base(nodeContext, "expression", theDefault)
    {
...
${cases}
    ...
    public SwitchNumeric(NodeContext nodeContext, ShaderNode<int> theCheck, IEnumerable<ShaderNode<T>> inputs,
        ShaderNode<T> theDefault) : base(nodeContext, "numericSwitch")
    {
        _check = theCheck;
        ...
        if (theDefault != null) myIns.Add(theDefault);
    ...
    {
        return ShaderNodesUtil.Evaluate(ShaderCode,
            new Dictionary<string, string> { { "cases", BuildCases(_cases, _default) } });
    }
}
...
public class Compare<T> : ShaderNode<bool>
{
    ...
    public Compare(NodeContext nodeContext, string theOperator, ShaderNode<T> theA, ShaderNode<T> theB,
        bool theAll = true) : base(nodeContext, "Assign")
    {
    ...
    protected override string SourceTemplate()
    {
        if (TypeHelpers.GetDimension<T>() == 1)
            return ShaderNodesUtil.Evaluate("${resultType} ${resultName} = ${a}${operator}${b};",
                new Dictionary<string, string>
                {
                    { "a", _a.ID },
                    { "operator", _operator },
        ...
        return ShaderNodesUtil.Evaluate("${resultType} ${resultName} = ${combine}(${a}${operator}${b});",
            new Dictionary<string, string>
            {
                { "a", _a.ID },
                { "operator", _operator },
