...
public class TextureGetDimensions : ShaderNode<GpuVoid>
{
    ...
    public TextureGetDimensions(NodeContext nodeContext, int dimension, ITextureInputProvider theTextureProvider,
        ShaderNode<int> mipLevel) : base(nodeContext, "textureGetDimension")
    {
        OptionalOutputs = [];
...
public class TextureFunction<T> : ShaderNode<T>
{
    ...
    protected override string SourceTemplate()
    {
        if (ShaderNodesUtil.HasNullValue(_arguments) || _texture == null) return GenerateDefaultSource();
