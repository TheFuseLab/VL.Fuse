...
{
    protected AbstractTypedFunction(NodeContext nodeContext, string theName, ShaderNode<TOut> theDefault = null) : base(
        nodeContext, theName, theDefault)
    {
...
${structMembers}
        ...
        {
            call.Append("        " + TypeHelpers.GetGpuType(property.PropertyType) + " " + property.Name + ";" +
                        Environment.NewLine);
        });
...
public class TypedBufferGet<T> : AbstractTypedFunction<T, T> where T : unmanaged
{
    ...
    public TypedBufferGet(NodeContext nodeContext, ShaderNode<Buffer<T>> theBuffer, ShaderNode<int> theIndex,
        ShaderNode<T> theDefault) : base(nodeContext, "getBuffer", theDefault)
    {
        _buffer = theBuffer;
        _index = theIndex;
...
public class TypedBufferSet<TIn> : AbstractTypedFunction<TIn, GpuVoid>, IComputeVoid where TIn : unmanaged
{
    ...
    public TypedBufferSet(NodeContext nodeContext, ShaderNode<Buffer<TIn>> theBuffer, ShaderNode<int> theIndex,
        ShaderNode<TIn> theValue) : base(nodeContext, "setBuffer")
    {
        _buffer = theBuffer;
        _index = theIndex;
        _value = theValue;
...
public class TypedBufferAppend<T> : AbstractTypedFunction<T, GpuVoid> where T : unmanaged
{
    ...
    public TypedBufferAppend(NodeContext nodeContext, ShaderNode<Buffer<T>> theBuffer, ShaderNode<T> theValue) : base(
        nodeContext, "appendBuffer")
    {
        _buffer = theBuffer;
        _value = theValue;
...
public class TypedBufferConsume<T> : AbstractTypedFunction<T, T> where T : unmanaged
{
    ...
    public TypedBufferConsume(NodeContext nodeContext, ShaderNode<Buffer<T>> theBuffer, ShaderNode<T> theDefault) :
        base(nodeContext, "consumeBuffer", theDefault)
    {
        _buffer = theBuffer;
