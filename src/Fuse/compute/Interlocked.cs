...
public class InterlockedBuffer<T> : ShaderNode<GpuVoid>, IComputeVoid
{
    ...
    public InterlockedBuffer(NodeContext nodeContext, string theFunction, IBufferInput<T> theBuffer,
        ShaderNode<int> theIndex, ShaderNode<T> theValue) : base(nodeContext, "interlocked")
    {
    ...
    protected override string SourceTemplate()
    {
        ...
        else
        {
            ...
            for (var i = 0; i < TypeHelpers.GetDimension<T>(); i++)
                shaderCode += "${function}(${bufferName}[${index}]." + components[i] + ", ${value}." + components[i] +
                              ", ${lastValue}." + components[i] + ");\n";
            return ShaderNodesUtil.Evaluate(shaderCode, new Dictionary<string, string>
            {
                { "function", _function },
                { "bufferName", _buffer.ID },
...
public class InterlockedTexture<T, TIndex> : ShaderNode<GpuVoid>, IComputeVoid
{
    ...
    protected override string SourceTemplate()
    {
        ...
        else
        {
            ...
            var shaderCode = "";
            for (var i = 0; i < TypeHelpers.GetDimension<T>(); i++)
                shaderCode += "${function}(${textureName}[${index}]." + components[i] + ", ${value}." + components[i] +
                              ", ${lastValue}." + components[i] + ");\n";
            return ShaderNodesUtil.Evaluate(shaderCode, new Dictionary<string, string>
            {
                { "function", _function },
