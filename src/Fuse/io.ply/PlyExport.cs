        ...
        string outputPlyPath)
    {
        ...
        for (var arrayIdx = 0; arrayIdx < vertexDataList.Count; arrayIdx++)
        {
            ...
            for (var i = 0; i < vertexCount; i++)
            {
                ...
                var worldPos = applyTransforms
                    ? Vector4.Transform(new Vector4(localPos, 1f), transform)
                    : new Vector4(localPos, 1f);
                ...
                for (var k = 0; k < stride; k++) mergedKeptVertices[start + k] = vertexData[baseIndex + k];
        ...
        var finalVertexCount = mergedKeptVertices.Count / stride;
        ...
        using (var writer = new BinaryWriter(fs))
        {
            ...
            writer.Write(Encoding.ASCII.GetBytes($"element vertex {finalVertexCount}\n"));

            // For each attribute name, write "property float <name>"
            foreach (var attr in elementNames) writer.Write(Encoding.ASCII.GetBytes($"property float {attr}\n"));

            writer.Write(Encoding.ASCII.GetBytes("end_header\n"));

            // -- Binary vertex data
            // Each vertex has 'stride' floats
            for (var i = 0; i < mergedKeptVertices.Count; i++)
            {
                var val = mergedKeptVertices[i];
                writer.Write(val);
            }
        }
