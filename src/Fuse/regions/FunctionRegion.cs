...
public class FunctionRegion
{
    ...
    {
        public RegionFunctionParameter(NodeContext nodeContext, ShaderNode<T> theType, int theId = 0) : base(
            nodeContext, "argument")
        {
            /*
    ...
    public sealed class RegionFunction<T> : AbstractFunction<T>
    {
        ...
        public RegionFunction(NodeContext nodeContext,
            ...
            IEnumerable<InputModifier> theModifiers = null) : base(nodeContext, "patchedFunction", theArguments,
            theModifiers, theDefault, theIsGroupable)
        {
            _name = theName;
...
${functionImplementation}
        ...
        private void HandleDelegates(IEnumerable<IDelegate> theDelegates,
            IDictionary<string, string> theFunctionValueMap)
        {
            theDelegates?.ForEach(delegateNode =>
            {
                ...
                delegateNode.PropertiesForTree().ForEach(kv => { AddProperties(kv.Key, kv.Value); });
            });
        }
        ...
        private static string BuildSignature(IEnumerable<AbstractShaderNode> inputs)
        {
            ...
            inputs.ForEach(input =>
                stringBuilder.Append(TypeHelpers.GetSignature(input.GetType().GetGenericArguments()[0])));
