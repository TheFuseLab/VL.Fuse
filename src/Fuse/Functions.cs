...
public abstract class AbstractFunction<T> : ShaderNode<T>
{
    ...
    {
        var resultPart = "";
        if (!TypeHelpers.IsVoidNode(this)) resultPart = "${resultType} ${resultName} = ";
...
public class IntrinsicFunction<T> : AbstractFunction<T>
{
    public IntrinsicFunction(
        ...
        IEnumerable<AbstractShaderNode> theArguments,
        bool theIsGroupable = false,
        IEnumerable<InputModifier> theModifiers = null) : base(nodeContext, theFunction, theArguments, theModifiers,
        theDefault, theIsGroupable)
    {
    }
}
...
public class MixinFunction<T> : AbstractFunction<T>
{
    public MixinFunction(
        ...
        int theGroupOptions = 0,
        IEnumerable<InputModifier> theModifiers = null) : base(nodeContext, theFunction, theArguments, theModifiers,
        theDefault, theIsGroupable, theGroupOptions)
    {
...
public sealed class CustomFunction<T> : AbstractFunction<T>
{
    ...
    private void UpdateMixins(IEnumerable<string> theMixins)
    {
        ...
        foreach (var mixin in theMixins) mixinList.Add(mixin);
    ...
    private void HandleDelegates(IDictionary<string, IDelegate> theDelegates,
        IDictionary<string, string> theFunctionValueMap)
    {
        ...
        foreach (var delegateNode in theDelegates)
        {
            ...
            delegateNode.Value.PropertiesForTree().ForEach(kv => { AddProperties(kv.Key, kv.Value); });
        }
    }
