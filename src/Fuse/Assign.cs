...
public class AssignValue<T> : ShaderNode<GpuVoid>, IComputeVoid
{
    ...
    public AssignValue(NodeContext nodeContext, ShaderNode<T> theTarget, ShaderNode<T> theSource) : base(nodeContext,
        "Assign")
    {
        _target = theTarget;
...
public class SetAbstract : ShaderNode<GpuVoid>, IComputeVoid
{
    ...
    public SetAbstract(NodeContext nodeContext, AbstractShaderNode theTarget, AbstractShaderNode theSource) : base(
        nodeContext, "Assign")
    {
        _target = theTarget;
        _target.WriteCounter++;
...
public class SetMember<T> : ShaderNode<GpuVoid>, IComputeVoid
{
    ...
    public SetMember(NodeContext nodeContext, ShaderNode<T> theObject, string theMember, AbstractShaderNode theValue) :
        base(nodeContext, "SetMember")
    {
        _object = theObject;
        _member = theMember;
        _value = theValue;
...
public class SetItem<T, TValue> : ShaderNode<GpuVoid>, IComputeVoid where TValue : struct
{
    ...
    public SetItem(NodeContext nodeContext, ShaderNode<T> theTarget, ShaderNode<int> theIndex,
        ShaderNode<TValue> theValue) : base(nodeContext, "setItem")
    {
        _target = theTarget;
        _index = theIndex;
        _value = theValue;
