...
public class GetMember<TIn, TOut> : ResultNode<TOut>
{
        ...
        ShaderNode<TOut> theDefault = null) : base(nodeContext, "GetMember", theDefault)
    {
        _member = theMember;
        _input = theInput;
        if (theInput != null && theMember != null) Name = theInput.Name + ShaderNodesUtil.FirstLetterToUpper(theMember);
...
public class GetItem<TIn, TOut> : ResultNode<TOut>
{
    ...
    public GetItem(NodeContext nodeContext, ShaderNode<TIn> theInput, ShaderNode<int> theIndex) : base(nodeContext,
        "GetItem")
    {
        _input = theInput;
        _index = theIndex;
...
public class GetItemAbstract<TOut> : ResultNode<TOut>
{
    ...
    public GetItemAbstract(NodeContext nodeContext, AbstractShaderNode theInput, ShaderNode<int> theIndex) : base(
        nodeContext, "GetItem")
    {
        _input = theInput;
        _index = theIndex;
...
{
    public Reorder3D(NodeContext nodeContext, ShaderNode<Vector3> theInput, Reorder3DMode theMember,
        ShaderNode<Vector3> theDefault = null) : base(nodeContext, theInput, theMember.ToString().ToLower(), theDefault)
    {
    }
}