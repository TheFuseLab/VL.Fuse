...
public delegate void Delegate2In2OutUpdate<TIn1, TIn2, TOut1, TOut2>(object stateInput, out object stateOutput,
    ShaderNode<TIn1> input1, ShaderNode<TIn2> input2, out ShaderNode<TOut1> output1, out ShaderNode<TOut2> output2);
...
public class Delegate2In2Out<TIn1, TIn2, TOut1, TOut2> : DelegateStateful<Delegate<TOut1>>
{
    ...
    public void Update(Delegate2In2OutCreate create, Delegate2In2OutUpdate<TIn1, TIn2, TOut1, TOut2> update)
    {
        ...
        var outputParameter =
            new FunctionParameter<TOut2>(nodeSubContextFactory.NextSubContext(), null, InputModifier.Out, 1);
        var set = new AssignValue<TOut2>(nodeSubContextFactory.NextSubContext(), outputParameter, outArgument2);
        var do2 = new Do2<TOut1>(nodeSubContextFactory.NextSubContext(), outArgument1,
...
public class Delegate2In2OutInvoke<TIn1, TIn2, TOut1, TOut2> : Invoke<TOut1>
{
    public Delegate2In2OutInvoke(
        ...
        ShaderNode<TIn2> theInput2) : base(nodeContext, theDelegate?.Delegate,
        new List<AbstractShaderNode> { theInput1, theInput2 })
    {
    }
}