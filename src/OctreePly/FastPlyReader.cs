...
public class FastPlyReader
{
    ...
    {
        { PropertyType.Float, ptr => { unsafe { return *(float*)ptr; } } },
        { PropertyType.Double, ptr => { unsafe { return (float)(*(double*)ptr); } } },
        { PropertyType.Int, ptr => { unsafe { return *(int*)ptr; } } },
        { PropertyType.UInt, ptr => { unsafe { return *(uint*)ptr; } } },
        { PropertyType.Short, ptr => { unsafe { return *(short*)ptr; } } },
        { PropertyType.UShort, ptr => { unsafe { return *(ushort*)ptr; } } },
        { PropertyType.Char, ptr => { unsafe { return *(sbyte*)ptr; } } },
        { PropertyType.UChar, ptr => { unsafe { return *(byte*)ptr; } } }
    };
    ...
    public static async Task LoadInBackgroundAsync(string filePath, ProgressInfo progressInfo)
    {
        ...
        try
        {
            ...
            int bufferSize = totalFileSize > 1_000_000_000 ? 16 * 1024 * 1024 : 4 * 1024 * 1024; // 16MB for >1GB files
            ...
            var (properties, vertexCount, headerSize) = await ParseHeaderOptimizedAsync(fileStream, progressInfo, startTime);
            ...
            for (int i = 0; i < properties.Count; i++)
            {
                ...
                {
                    scalarFields[prop.Name] = GC.AllocateArray<float>(vertexCount, pinned: true);
                });
            }
    ...
    private static async Task<(List<PropertyInfo> properties, int vertexCount, long headerSize)> ParseHeaderOptimizedAsync(
        ...
        ProgressInfo progressInfo, DateTime startTime, long totalFileSize, int bufferSize)
    {
        ...
        int maxDegreeOfParallelism = Math.Min(Environment.ProcessorCount, 8); // Cap at 8 threads
        int verticesPerChunk = Math.Max(1000, vertexCount / (maxDegreeOfParallelism * 4)); // Ensure reasonable chunk sizes
        
        // Pre-compile property readers for each property
        var readers = new PropertyReader[properties.Length];
        for (int i = 0; i < properties.Length; i++)
        {
            readers[i] = PropertyReaders[properties[i].Type];
        }
        ...
        {
            int chunkVertices = Math.Min(verticesPerChunk, vertexCount - processedVertices);
            int chunkBytes = chunkVertices * vertexSize;
            
            // Read chunk data
            byte[] chunkBuffer = new byte[chunkBytes];
            int bytesRead = await stream.ReadAsync(chunkBuffer, 0, chunkBytes);
            if (bytesRead == 0) break;
            
            int actualVertices = bytesRead / vertexSize;
            int chunkStartVertex = processedVertices;
            ...
            {
                double progressPercent = 10 + (double)processedVertices / vertexCount * 90;
        ...
        PropertyInfo[] properties, int startVertex, int vertexCount, int vertexSize)
    {
        ...
        fixed (float* zPtr = &zArray[startVertex])
        {
            ...
            {
                byte* vertexPtr = bufferPtr + (v * vertexSize);
