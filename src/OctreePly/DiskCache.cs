...
namespace VL.E57
{
    ...
    {
        public static string BasePath { get; set; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "VL.Fuse", "OctreePlyCache");
    ...
    public sealed class DiskCache
    {
        ...
        {
            public static int OctreeChunkSizeBytes { get; set; } = 64 * 1024 * 1024; // 64 MB (not used in new Octree read, but kept for compatibility)
        ...
        internal static async Task ReadAtExactlyAsync(SafeFileHandle handle, Memory<byte> destination, long fileOffset, CancellationToken ct)
        {
            int read = 0;
            while (read < destination.Length)
            {
                var r = await System.IO.RandomAccess.ReadAsync(handle, destination.Slice(read), fileOffset + read, ct);
                if (r == 0) break;
                read += r;
            }
        }

        internal static async Task ReadFileParallelRandomAccessAsync(SafeFileHandle handle, byte[] destination, int chunkSize, int degree, CancellationToken ct, string label = "")
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(label))
                {
                    ...
                    Console.WriteLine($"[DiskCache:{label}] Read {mb:F1} MB in {sw.Elapsed.TotalMilliseconds:F0} ms ({mbps:F1} MB/s)");
                }
            }
            catch { }
        }

        private static async Task ReadChunkAsync(SafeFileHandle handle, byte[] destination, int start, int size, CancellationToken ct)
        {
            var mem = destination.AsMemory(start, size);
            int offset = 0;
            while (offset < size)
            {
                var r = await System.IO.RandomAccess.ReadAsync(handle, mem.Slice(offset), start + offset, ct);
                if (r == 0) break;
                offset += r;
            }
        }
        ...
        internal static void ReadAtExactly(SafeFileHandle handle, Span<byte> destination, long fileOffset, CancellationToken ct)
        {
            int read = 0;
            while (read < destination.Length)
            {
                ct.ThrowIfCancellationRequested();
                var r = System.IO.RandomAccess.Read(handle, destination.Slice(read), fileOffset + read);
                if (r == 0) break;
                read += r;
            }
        }
        ...
        internal static void TryLogEnvironment(string tag)
        {
            try
            {
                ...
                if (di != null)
                {
                    Console.WriteLine($"[DiskCache:{tag}] BasePath={basePath} Drive={di.Name} Free={di.AvailableFreeSpace / (1024.0 * 1024.0 * 1024.0):F1}GB");
                }
                else
                {
        ...
        public static async Task<T> GetOrCreateAsync<T>(string cacheNamespace, string key, ICacheSerializer<T> serializer, Func<CancellationToken, Task<T>> buildAsync, CancellationToken ct)
        {
        ...
        public static bool TryGet<T>(string cacheNamespace, string key, ICacheSerializer<T> serializer, CancellationToken ct, out Task<T> payloadTask)
        {
            var entry = GetEntryDir(cacheNamespace, key);
            var manifestPath = Path.Combine(entry, "manifest.json");
            if (!File.Exists(manifestPath))
        ...
        {
            var entry = GetEntryDir(cacheNamespace, key);
            try { if (Directory.Exists(entry)) Directory.Delete(entry, true); } catch { }
        }
        ...
        static string GetEntryDir(string cacheNamespace, string key)
        {
        ...
        static async Task<Manifest?> ReadManifestAsync(string path, CancellationToken ct)
        {
        ...
        static async Task WriteManifestAsync(string path, Manifest manifest, CancellationToken ct)
        {
        ...
        static Manifest? ReadManifestSync(string path)
        {
            try
            {
        ...
        static async Task Touch(string manifestPath, Manifest man, CancellationToken ct)
        {
            try
            {
        ...
        static async Task AtomicSwapAsync(string tmpEntryDir, string finalEntryDir)
        {
            ...
            {
                try { Directory.Delete(finalEntryDir, true); } catch { }
            }
    ...
    public sealed class PlyArraysCacheSerializer : DiskCache.ICacheSerializer<Dictionary<string, float[]>>
    {
        ...
        const string DataFile = "arrays.bin";
        const string TocFile = "arrays.toc.json";
        ...
        public async Task WriteAsync(string entryDir, Dictionary<string, float[]> payload, CancellationToken ct)
        {
            ...
            {
                var bytes = (long)kv.Value.LongLength * sizeof(float);
                toc.Add(new TocEntry { Name = kv.Key, Offset = offset, LengthFloats = kv.Value.LongLength });
                offset += bytes;
            }

            await using (var fs = new FileStream(Path.Combine(entryDir ?? string.Empty, DataFile), FileMode.Create, FileAccess.Write, FileShare.None, 1024 * 1024, true))
            {
                foreach (var kv in payload)
                {
                    var arr = kv.Value;
            ...
            await using (var fs = new FileStream(Path.Combine(entryDir ?? string.Empty, TocFile), FileMode.Create, FileAccess.Write, FileShare.None, 4096, true))
            {
        ...
        public async Task<Dictionary<string, float[]>> ReadAsync(string entryDir, CancellationToken ct)
{
    ...
    using (var fs = new FileStream(tocPath, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, useAsync: false))
    {
    ...
    bool canUseSingleBuffer = length <= int.MaxValue;
    if (canUseSingleBuffer)
    {
        foreach (var entry in toc)
        {
            long fieldBytes = entry.LengthFloats * sizeof(float);
            long end = entry.Offset + fieldBytes;
            if (end > int.MaxValue)
            {
    ...
    if (!DiskCache.s_loggedPlySettings)
    {
        ...
        {
            var top = toc
                .Select(t => new { Name = t.Name ?? "values", MB = (t.LengthFloats * sizeof(float)) / (1024.0 * 1024.0) })
                .OrderByDescending(x => x.MB)
                .Take(3)
                .ToArray();
            Console.WriteLine($"[DiskCache:PLY] Top fields by size: {string.Join(", ", top.Select(x => x.Name + ":" + x.MB.ToString("F1") + "MB"))}");
        }
        catch { }
    }
    ...
    else
    {
        ...
        // ---------------------------------------------------
        Console.WriteLine("[DiskCache:PLY] arrays.bin > 2GB or offsets exceed Int32; using per-field streaming fallback (direct-to-float).");
        ...
        foreach (var entry in toc)
        {
            ...
            var byteSpan = System.Runtime.InteropServices.MemoryMarshal.AsBytes(floats.AsSpan());
            int byteLength = byteSpan.Length;
    ...
    return result;
        ...
        private static void ReadFloatArray(Stream data, float[] destination, CancellationToken ct)
        {
            ...
            var bytesSpan = System.Runtime.InteropServices.MemoryMarshal.AsBytes(destination.AsSpan());
            int read = 0;
    ...
    public sealed class OctreeCacheSerializer : DiskCache.ICacheSerializer<OctreeCacheSerializer.Payload>
    {
        ...
        {
            public Payload(byte[] nodeBuffer, byte[] indexBuffer, int nodeCount, int indexCount, long totalMemory, bool hasPrecomputedLODs)
            {
        ...
        const string Nodes = "nodes.bin";
        const string Indices = "indices.bin";
        const string Meta = "meta.json";

        public async Task WriteAsync(string entryDir, Payload payload, CancellationToken ct)
        {
            await File.WriteAllBytesAsync(Path.Combine(entryDir ?? string.Empty, Nodes), payload.NodeBuffer ?? Array.Empty<byte>(), ct);
            await File.WriteAllBytesAsync(Path.Combine(entryDir ?? string.Empty, Indices), payload.IndexBuffer ?? Array.Empty<byte>(), ct);
            await using var fs = new FileStream(Path.Combine(entryDir ?? string.Empty, Meta), FileMode.Create, FileAccess.Write, FileShare.None, 4096, true);
        ...
        public async Task<Payload> ReadAsync(string entryDir, CancellationToken ct)
        {
            ...
            if (!DiskCache.s_loggedOctreeSettings)
            {
                ...
                Console.WriteLine($"[DiskCache:Octree] Sizes: nodes={nodeBuffer.LongLength / (1024.0 * 1024.0):F1}MB indices={indexBuffer.LongLength / (1024.0 * 1024.0):F1}MB");
            }
            ...
            try
            {
                ...
                Console.WriteLine($"[DiskCache:Octree] Read {totalMB:F1} MB in {sw.Elapsed.TotalMilliseconds:F0} ms ({mbps:F1} MB/s)");
            }
            catch { }

            MetaInfo? meta;
            await using (var fs = new FileStream(Path.Combine(entryDir ?? string.Empty, Meta), FileMode.Open, FileAccess.Read, FileShare.Read, 4096, true))
            {
        ...
        public async Task<string> ComputeSha256Async(string entryDir, CancellationToken ct)
        {
            ...
            byte[] hash1 = Array.Empty<byte>();
            byte[] hash2 = Array.Empty<byte>();
            if (File.Exists(path1))
            {
                await using var fs1 = new FileStream(path1, FileMode.Open, FileAccess.Read, FileShare.Read, 1024 * 1024, true);
                hash1 = await sha.ComputeHashAsync(fs1, ct);
            }
            if (File.Exists(path2))
            {
                await using var fs2 = new FileStream(path2, FileMode.Open, FileAccess.Read, FileShare.Read, 1024 * 1024, true);
    ...
    public static class DiskCacheKey
    {
        ...
        public static string ForOctree(string pointCloudKey, int maxDepth, int maxPointsPerLeaf, float subdivisionEpsilon, bool optimizeForSpeed, bool enableDetailedValidation)
        {
            var s = $"pc={pointCloudKey}|md={maxDepth}|mpl={maxPointsPerLeaf}|eps={subdivisionEpsilon}|opt={optimizeForSpeed}|veri={enableDetailedValidation}";
            return s;
        }
    }
}
