...
public static class Utils
{
    ...
    )
    {
        ...
        var ro = viewerPosition;
        var rd = Vector3.Normalize(particlePos - viewerPosition);
        ...
        if (Math.Abs(p.Y - boxMin.Y) < eps) FaceIndex = 0;
        else if (Math.Abs(p.Y - boxMax.Y) < eps) FaceIndex = 1;
        else if (Math.Abs(p.X - boxMax.X) < eps) FaceIndex = 2;
        else if (Math.Abs(p.Z - boxMax.Z) < eps) FaceIndex = 3;
        else if (Math.Abs(p.X - boxMin.X) < eps) FaceIndex = 4;
        else if (Math.Abs(p.Z - boxMin.Z) < eps) FaceIndex = 5;
